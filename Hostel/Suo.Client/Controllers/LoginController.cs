...
namespace Suo.Client.Controllers
{
    public class LoginController : Controller
    {
        ...
        public IActionResult Index(string redirectUri)
        {
            ...
            /*_ = _authenticationService.LogOut(tokenModel, this.HttpContext);*/
        ...
        public async Task<IActionResult> Login(string email, string password, string redirectUri)
        {
            ...
            if (resualt.IsSuccsesdTokenLogin)
            {
                ...
                {
                    await _authenticationService.LogOut(tokenModel, this.HttpContext);
                    //await _authenticationService.LoginToBlazor(tokenModel);
                    
                    return Redirect(_appConfiguration.AdminProjectUrl);//редирект на backend
                }
                return Redirect("/");//сделай тут заход снова*/
            }
            else
            {
        ...
        /* var token = GetPrincipalFromExpiredToken(resualt.Token);
         ...
         var answer = token.Claims.FirstOrDefault(x => x.Type == ClaimsIdentity.DefaultRoleClaimType).Value;
                 if (answer != "User" )
                 {
                     resualt.FailMessage = answer.ToString();
                     return View("Index", resualt);//редирект на backend
     }
                 *//*var answer = token.Claims.FirstOrDefault(x => x.Value == "SysAdmin"  x.Value == "Admin");
                 resualt.FailMessage = answer.ToString();
                 //Сломано - чини*//*
                 return Redirect(redirectUri);//сделай тут заход снова*/
